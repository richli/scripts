...
# For all the git annex objects with the 3-character directory hash, create links from 2-character directory hash to 3-character hash
...
# where abc and def are three-character values generated differently from the key MD5

# The problem is the files in the git repo are symlinks to the annex, but they use the two-character directory style, whereas if the files are stored via rsync, they're in the three-character directory style.
# This script symlinks the files in the three-character directories to where they *should* be, in the two-character directories
